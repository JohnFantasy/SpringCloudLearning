...
1. 对象头
   ...
   **在32位操作系统中，对象头markword占4个字节32位、类型指针占4个字节32位共64位**
   
   **在64位操作系统中，对象头markword占8个字节64位、类型指针占8个字节64位共128位**
...
3. 对齐填充
   ...
   **可能有可能没有，如果Java对象的整体大小是8字节的整数倍则不需要填充，否则需要填充为8字节的整数倍，以方便操作系统寻址。**
   
   ## 1、偏向锁
   
   偏向锁是Java1.6中的重要引进，HptSpot虚拟机作者经过研究发现，在大多数情况下，锁不仅不存在多线程的竞争，而且总是由同一个线程多次获得，为了让线程获得锁的成本更低，引进了偏向锁。所谓的“偏”是指：这个锁会偏向于第一个获得它的线程，会在对象头中存储偏向的线程的ID，后面该线程进入和退出同步代码块时只需要检查是否时偏向锁、锁标志位、threadId即可。不过一旦出现多线程竞争的情形，就必须撤销偏向锁，所以撤销偏向锁的性能必须小于之前节省下来的CAS原子操作的性能消耗，不然就得不偿失了。
   
   ## 偏向锁的原理
   
   ## 2、轻量级锁
   
   JDK1.6之后引入的新的锁机制。目的：在多线程交替执行的情况下，尽量避免多线程重量级锁的性能问题。
